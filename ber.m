...
%     bits = f_b * T;
    ...
    %pskModulator = pskmod(4,-pi/4,'BitInput',true);
    %pskDemodulator = comm.PSKDemodulator(4,-pi/4,'BitOutput',true);
    %pskDemodulator = pskdemod('ModulationOrder',4,...
...
%                 b_2hat = [b_2hat (imag(yEq)<0)];
                ...
                channel=pskdemod(yEq,4,-pi/4) ;
                channel_llrs = [channel_llrs; channel];
                
                rxData = pskdemod(yEq,4,-pi/4) < 0;
                b_hat = [b_hat; rxData];
            end
...
hold on
    ber_perfCsi = ber1;
    semilogy(linspace(0,1,length(ber1)),ber1)
    legend('OFDM system')
    xlabel('SNR [dB]')
    ylabel('BER')
    hold off
    ...
    figure(6);
    hold on
    semilogy(linspace(0,1,length(bler1)), bler1)
