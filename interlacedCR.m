...
%         tic
        for v = 1:num_v
            for i=Columns{v}
                H5cv(i,Rows{i}) = check_node(H5vc(i,Rows{i}));
            end
            H5vc(Columns{v},v) = Channel(v) + sum(H5cv(Columns{v},v)) - H5cv(Columns{v},v);
        end
    HplusC = [Channel;H5cv];
    sums = sum(HplusC);
...
%         tic
    ...
    H6plusC = [Channel;H6cv];
    sums = sum(H6plusC);% was HplusC
    xhat6 = zeros(1,num_v);
    xhat6(sums<0) = 1;
    ...
    if syndrome_output6 == 0
        if err6 == 0
            count_icm_correct = count_icm_correct + 1;
        else
            count_icm_wrong = count_icm_wrong + 1;
        end
    end
...
disp(["Correct count flooding:" count_s_correct]);
disp(["Wrong count flooding:" count_s_wrong]);
disp(["Wrong count serial_c:" count_m_wrong]);
disp(["Correct count serial_c:" count_m_correct]);
disp(["Wrong count serial_v:" count_w_wrong]);
disp(["Correct count serial_v SPA:" count_w_correct]);
disp(["Correct count RMP:" count_fm_correct]);
disp(["Wrong count RMP:" count_fm_wrong]);
disp(["Correct count CMP:" count_scm_correct]);
disp(["Wrong count CMP:" count_scm_wrong]);
disp(["Correct count I-CRMP:" count_icm_correct]);
disp(["Wrong count I-CRMP:" count_icm_wrong]);
...
legend("Flooding", "Serial_c", "Serial_v","RMP","CMP","I-CRMP","Raw");
hold off
