...
%         toc
    ...
    while(syn ~=0)
        ...
        min_dis = repmat(q',1,num_v) + R;
        min_dis(min_dis==1)=0;
        min_dis = sum(min_dis);
        %min_dis = min_dis-sum(H2);
        min_cod = 1000*ones(1,num_v);
        min_cod(min_dis == max(min_dis)) = abs(sums(min_dis == max(min_dis)));
        sums(abs(sums)==min(min_cod)) = -sums(abs(sums)==min(min_cod));  
        yhat = zeros(1,num_v);
        yhat(sums<0) = 1;
        q = (mod(yhat*matrix.',2));
        if sum(q)>syn
    ...
    bfpp(Noise_lvls,l)=sum(abs(yhat-codeword));
...
semilogy(scale,BERflood_RBAlpha,"-o")
hold on
semilogy(scale,rBER,"-x")
semilogy(scale,BFpP,"-diamond")
title('Bit Error Rate for AWGN Channel comparison')
xlabel('SNR')
ylabel('Bit Error Rate')
legend("serial c","raw","BF")
hold off
